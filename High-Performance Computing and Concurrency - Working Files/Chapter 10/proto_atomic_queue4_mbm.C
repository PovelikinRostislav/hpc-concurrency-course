// Wait-free queue with begin and end indices, testing for the entry being initialized, one consumer thread
#include <string.h>
#include <atomic>
#include <memory>
#include <mutex>
...
class concurrent_queue {
  ...
  bool get(entry_t& x) {
    ...
    if (i >= end_.load(std::memory_order_relaxed)) {
        begin_.fetch_sub(1, std::memory_order_relaxed);
        return false;
    }
    if (mem_[i & mask].x.load(std::memory_order_acquire) == 0) {
        begin_.fetch_sub(1, std::memory_order_relaxed);
        return false;
    }
    x = mem_[i & mask];

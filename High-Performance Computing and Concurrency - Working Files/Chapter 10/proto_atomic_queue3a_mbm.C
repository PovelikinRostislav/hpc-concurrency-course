// Lock-free queue with begin, writers' end, and readers' end indices, CAS on begin
#include <string.h>
#include <atomic>
#include <memory>
#include <mutex>
...
class concurrent_queue {
  ...
  public:
  concurrent_queue() : mem_((entry_t*)malloc(capacity*sizeof(entry_t))), begin_(0), end_(0), endr_(0) {}
  ...
  bool add(const entry_t& x) {
    ...
    endr_.fetch_add(1, std::memory_order_release);
    return true;
  }
  bool get(entry_t& x) {
    int s = 0;
    ...
    do {
      if (i >= endr_.load(std::memory_order_acquire)) return false;
    } while (!begin_.compare_exchange_strong(i, i + 1, std::memory_order_relaxed) && wait(s));
    x = mem_[i & mask];
    return true;
  }
  size_t size() {
